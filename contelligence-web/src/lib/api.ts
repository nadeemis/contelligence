...
export const scheduleApi = {
  ...
  history: (id: string, limit = 20) =>
    apiFetch<ScheduleRunRecord[]>(`/schedules/${id}/history?limit=${limit}`),
};
...
// Re-export for SSE streaming URL construction
export { BASE_URL };
...
export const skillsApi = {
  ...
  uploadFile: async (id: string, file: File, path: string) => {
    ...
    const res = await fetch(`${BASE_URL}/skills/${id}/files`, {
      method: "POST",
      body: formData,
    });
    if (!res.ok) {
  ...
  uploadZip: async (id: string, file: File) => {
    const formData = new FormData();
    formData.append("file", file);
    const res = await fetch(`${BASE_URL}/skills/${id}/upload-zip`, {
      method: "POST",
      body: formData,
    });
    if (!res.ok) {
