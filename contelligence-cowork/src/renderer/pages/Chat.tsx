...
const Chat = () => {
  ...
  // Refetch session data when streaming ends so status badge updates
  useEffect(() => {
    if (prevIsStreamingRef.current && !isStreaming && sessionId) {
  ...
  // Derive effective status from live events when available
  const effectiveStatus = useMemo(() => {
    if (isStreaming) return "active";
    if (events.length > 0) {
      const lastEvent = events[events.length - 1];
      if (lastEvent.type === "done") return "completed";
      if (lastEvent.type === "error" && !(lastEvent as { recoverable?: boolean }).recoverable) return "failed";
    }
    if (session?.status) return session.status;
    return null;
  }, [isStreaming, events, session]);
